...
interface MailCraftContextType {
  ...
  handleCarregarTemplate: (template: {
    ...
    corpoHtml: string;
    anexos: any[];
  }) => void; // Pode ser necessário ajustar o tipo de anexos salvos
...
const MailCraftContext = createContext<MailCraftContextType | undefined>(undefined);

export const MailCraftProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  React.useEffect(() => {
    if (htmlTree) {
      setCorpoHtmlString(htmlTree.renderizar());
    }
  }, [htmlTree]);
  ...
  const importarContatos = useCallback(async (file: File) => {
    ...
    try {
      ...
      alert('Erro ao importar contatos. Verifique o formato do arquivo e se as colunas estão corretas (Nome, Email).');
    }
  }, []);

  const addHtmlElementToBody = useCallback((element: IHtmlElement) => {
  ...
  }, [ assunto, destinatarios, anexosSelecionados, htmlTree, emailBuilder, emailSender ]);
  ...
  const contextValue = useMemo(
    () => ({
      ...
      emailPrevia: htmlTree.renderizar(),
    }),
    [
      assunto,
      setAssunto,
